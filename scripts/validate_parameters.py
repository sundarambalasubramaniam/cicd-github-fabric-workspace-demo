"""Lightweight schema check for config/parameter.yml.
...
def fail(msg: str) -> None:
    print(f"::error file=config/parameter.yml::{msg}")
    sys.exit(1)
...
def main(path: str) -> None:
    ...
    for entry in data.get("find_replace", []) or []:
        ...
        if missing:
            fail(f"find_replace entry missing keys: {sorted(missing)} in {entry}")
        if not isinstance(entry["replace_value"], dict):
            fail(f"find_replace.replace_value must be a mapping per env: {entry}")

    for entry in data.get("key_value_replace", []) or []:
        missing = KV_REPLACE_REQUIRED - set(entry)
        if missing:
            fail(f"key_value_replace entry missing keys: {sorted(missing)} in {entry}")

    print(f"OK: {path}")


if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "config/parameter.yml")
